...
use iced::{
    Element, Length, keyboard::{Key, key::{Code, Named, Physical}}, widget::{Column, Container, ProgressBar, Row, Text, TextEditor, space, text_editor::{Binding, KeyPress}}
};
...
impl Chat {
    ...
    fn key_bindings(key_press: KeyPress) -> Option<Binding<super::Message>> {
        match key_press.clone().key {
            Key::Named(Named::Enter) if key_press.modifiers.shift() => Some(Binding::Enter),
            Key::Named(Named::Enter) => Some(Binding::Custom(super::Message::SendMessage)),
